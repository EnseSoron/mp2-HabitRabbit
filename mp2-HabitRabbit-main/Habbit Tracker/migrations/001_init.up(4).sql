...
-- USER HABITS (user config + cached progress)
...
-- HABIT LOGS (source of truth for completions)
...
CREATE TABLE habit_achievements (
    ...
    CONSTRAINT uniq_habit_achievement UNIQUE (user_habit_id, achievement_id)
);
...
CREATE INDEX idx_habit_achievements_user_habit_id ON habit_achievements(user_habit_id);
CREATE INDEX idx_habit_achievements_achievement_id ON habit_achievements(achievement_id);
...
-- Базовые достижения
INSERT INTO achievement_definitions (code, title, description, rule_type, rule_value) VALUES
    ('first_habit', 'Первая привычка', 'Ты создал свою первую привычку', 'habit_count', 1),
    ('first_checkin', 'Первый шаг', 'Ты впервые отметил выполнение привычки', 'completed_count', 1),
    ('streak_7', 'Серия 7 дней', 'Ты продержался 7 дней подряд', 'streak', 7),
    ('streak_30', 'Серия 30 дней', 'Ты продержался 30 дней подряд', 'streak', 30),
    ('complete_10', '10 выполнений', 'Ты выполнил привычку 10 раз', 'completed_count', 10),
    ('complete_50', '50 выполнений', 'Ты выполнил привычку 50 раз', 'completed_count', 50)
